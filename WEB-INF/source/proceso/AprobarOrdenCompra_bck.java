...
public class AprobarOrdenCompra extends GenericTransaction 
{
			...
			if(rsOrdenCompra.getRecordCount()>0)
			{
				...
				if(rsRol.getRecordCount()>0)
				{
					//**INYECCION DE VALORES EN EL VALIDATOR PROVENIENTES DE LA TABLA: smn_orden_compra_cabecera
						...
						{
							mensaje = "**error**, el monto_ml de la orden de compra es 0.0";
					...
					if(mca == 0)
					{
							...
							while(rsOrdenCompraDetalle.next() != false)
							{
								//** INYECCION DE VALORES AL VALIDATOR PROVENIENTES DE: smn_orden_compra_detalle.
									...
									if(rsitem.getRecordCount()<1)
									{
										...
										System.out.println(mensaje);
										return 7; //no se encontro el item o esta inactivo. 
									}
								...
								if(mde == 0)
								{
									...
									if(rsOrdenCompraImpuesto.getRecordCount()>0)
									{
										...
										//** INYECCION DE VALORES AL VALIDATOR PROVENIENTES DE: smn_orden_compra_impuesto.
											...
											if(rsOrdenCompraImpuesto.getString("smn_tipo_impuesto_rf") != null)
												inputParams.setValue("oci_tipo_impuesto_rf", rsOrdenCompraImpuesto.getInt("smn_tipo_impuesto_rf"));
											else
												inputParams.setValue("oci_tipo_impuesto_rf", 0);
									...
									if(rsOrdenCompraDesc_retenc.getRecordCount()>0)
									{
										...
										//** INYECCION DE VALORES AL VALIDATOR PROVENIENTES DE: smn_orden_compra_descuentos_deducciones.
											...
											if(rsOrdenCompraDesc_retenc.getString("mdd_monto_base_ml") != null)
												inputParams.setValue("mdd_monto_base_ml", rsOrdenCompraDesc_retenc.getDouble("mdd_monto_base_ml"));
											else
												inputParams.setValue("mdd_monto_base_ml", 0.0);
											
											if(rsOrdenCompraDesc_retenc.getString("smn_porcentaje_rf") != null)
												inputParams.setValue("smn_porcentaje_rf", rsOrdenCompraDesc_retenc.getInt("smn_porcentaje_rf"));
											else
												inputParams.setValue("smn_porcentaje_rf", 0);
											
											if(rsOrdenCompraDesc_retenc.getString("mdd_monto_descuento_ml") != null)
												inputParams.setValue("mdd_monto_descuento_ml", rsOrdenCompraDesc_retenc.getDouble("mdd_monto_descuento_ml"));
											else
											...
											if(rsOrdenCompraDesc_retenc.getString("mdd_monto_base_ma") != null)
												inputParams.setValue("mdd_monto_base_ma", rsOrdenCompraDesc_retenc.getDouble("mdd_monto_base_ma"));
											else
												inputParams.setValue("mdd_monto_base_ma", 0.0);
											
											if(rsOrdenCompraDesc_retenc.getString("mdd_monto_descuento_ma") != null)
												inputParams.setValue("mdd_monto_descuento_ma", rsOrdenCompraDesc_retenc.getDouble("mdd_monto_descuento_ma"));
											else
						...
						else
						{
							...
							rc = 2; //no se encontraron detalles de la orden de compra.
						}
					}
					else
					{
				...
				else
				{
					...
					rc = 8; //no se encontro un usuario CO.
				}	
			}
			else
			{
				mensaje = "la orden de compra no esta registrada";
				rc = 7; //la orden de compra no esta registrada.
			}
	...
	public int registrarMovimientoDetalle(Connection conn, Recordset inputParams) throws Throwable
	{
		...
		try
		{
			...
			{
				rc = 3; //no se registro movimiento cabecera.
			}
		}
		catch(Throwable e)
		{
	...
	public int registrarMovimientoDetalleImpuesto(Connection conn, Recordset inputParams) throws Throwable
	{
		...
		try
		{
			...
			{
				rc = 4; //no se registro movimiento cabecera.
			}
		}
		catch(Throwable e)
		{
	...
	public int registrarMovimientoDetalleDesc_retenc(Connection conn, Recordset inputParams) throws Throwable
	{
		...
		try
		{
			...
			if(rsMovimientoDetalleDesc_retenc.getRecordCount()>0)
			{
				...
				inputParams.setValue("'smn_movimiento_detalle_desc_ret_id'", rsMovimientoDetalleDesc_retenc.getInt("'smn_movimiento_detalle_desc_ret_id'"));
				
				rc = 0; // ejecucion del subproceso exitosamente.
			}
			else
			{
				rc = 5; //no se registro movimiento cabecera.
			}
		}
		catch(Throwable e)
		{
