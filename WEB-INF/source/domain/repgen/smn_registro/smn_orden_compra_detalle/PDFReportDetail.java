package domain.repgen.smn_registro.smn_orden_compra_detalle;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_contrato_modulo_id");
			if (d != null)
			{
			name  = "b_smn_contrato_modulo_id";
			label = "${lbl:b_smn_contrato_modulo_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_servicios_id");
			if (d != null)
			{
			name  = "b_smn_servicios_id";
			label = "${lbl:b_smn_servicios_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_contrato_modulo_id_name");
			if (d != null)
			{
			name  = "b_smn_contrato_modulo_id";
			label = "${lbl:b_smn_contrato_modulo_id}";
		...
		d = rs.getString("smn_linea_id_name");
			if (d != null)
			{
			name  = "b_smn_linea_id";
			label = "${lbl:b_smn_linea_id}";
