package domain.repgen.smn_registro.smn_orden_compra_fecha_entrega;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("ofe_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("ofe_consecutivo");
			if (d != null)
			{
			name  = "b_ofe_consecutivo";
			label = "${lbl:b_ofe_consecutivo}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ofe_cantidad");
			if (d != null)
			{
			name  = "b_ofe_cantidad";
			label = "${lbl:b_ofe_cantidad}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ofe_fecha_entrega");
			if (d != null)
			{
			name  = "b_ofe_fecha_entrega";
			label = "${lbl:b_ofe_fecha_entrega}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ofe_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
