package domain.repgen.smn_registro.smn_orden_compra_impuesto;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_cod_impuesto_deduc_rf_name");
			if (d != null)
			{
			name  = "b_smn_cod_impuesto_deduc_rf";
			label = "${lbl:b_smn_cod_impuesto_deduc_rf}";
		...
		d = rs.getString("smn_cod_impuesto_deduc_rf");
			if (d != null)
			{
			name  = "b_smn_cod_impuesto_deduc_rf";
			label = "${lbl:b_smn_cod_impuesto_deduc_rf}";
