package domain.repgen.smn_registro.smn_orden_compra_cabecera;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("occ_fecha_registro_ini");
			if (d != null)
			{
		...
		d = rs.getString("occ_fecha_orde_compra");
			if (d != null)
			{
			name  = "b_occ_fecha_orde_compra";
			label = "${lbl:b_occ_fecha_orde_compra}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_proveedor_id_name");
			if (d != null)
			{
			name  = "b_smn_proveedor_id";
			label = "${lbl:b_smn_proveedor_id}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("occ_orden_compra_numero");
			if (d != null)
			{
			name  = "b_occ_orden_compra_numero";
			label = "${lbl:b_occ_orden_compra_numero}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("occ_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date}@{\${def:date}";
			label = "${lbl:b_store_date}@{\${def:date}}";
