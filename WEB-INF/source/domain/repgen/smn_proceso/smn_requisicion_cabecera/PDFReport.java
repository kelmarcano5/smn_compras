package domain.repgen.smn_proceso.smn_requisicion_cabecera;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
			...
			if (d != null)
			{
			name  = "b_purchasing_documents";
			label = "${lbl:b_purchasing_documents}";
		...
		d = rs.getString("req_numero");
			if (d != null)
			{
			name  = "b_requisition_number";
			label = "${lbl:b_requisition_number}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("req_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("req_estatus");
			if (d != null)
			{
			name  = "b_requisition_status";
			label = "${lbl:b_requisition_status}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("req_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("req_descripcion");
			if (d != null)
			{
			name  = "b_requisition_description";
			label = "${lbl:b_requisition_description}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_tipo_documento_id");
			if (d != null)
			{
			name  = "b_purchasing_documents_types";
			label = "${lbl:b_purchasing_documents_types}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
