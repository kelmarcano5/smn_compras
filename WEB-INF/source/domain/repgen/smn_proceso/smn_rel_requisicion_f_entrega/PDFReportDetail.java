package domain.repgen.smn_proceso.smn_rel_requisicion_f_entrega;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("cfe_consecutivo");
			if (d != null)
			{
			name  = "b_consecutivel";
			label = "${lbl:b_consecutivel}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("cfe_fecha_de_entrega_fin");
			if (d != null)
			{
		...
		d = rs.getString("cfe_fecha_de_entrega_ini");
			if (d != null)
			{
			name  = "b_delivery_date";
			label = "${lbl:b_delivery_date}";
