package domain.repgen.smn_proceso.smn_requisicion_detalle;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_requisicion_cabecera_id_name");
			if (d != null)
			{
			name  = "b_headboard_requisition";
			label = "${lbl:b_headboard_requisition}";
		...
		d = rs.getString("smn_contrato_id");
			if (d != null)
			{
			name  = "b_contract";
			label = "${lbl:b_contract}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_requisicion_cabecera_id");
			if (d != null)
			{
			name  = "b_headboard_requisition";
			label = "${lbl:b_headboard_requisition}";
		...
		d = rs.getString("smn_cabecera_version_id_name");
			if (d != null)
			{
			name  = "b_version_headboard";
			label = "${lbl:b_version_headboard}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_linea_id");
			if (d != null)
			{
			name  = "b_lines}@{SELECT smn_compras.smn_lineas.smn_lineas_id as id, smn_compras.smn_lineas.lin_codigo||' - '|| smn_compras.smn_lineas.lin_nombre as item from smn_compras.smn_lineas";
			label = "${lbl:b_lines}@{SELECT smn_compras.smn_lineas.smn_lineas_id as id, smn_compras.smn_lineas.lin_codigo||' - '|| smn_compras.smn_lineas.lin_nombre as item from smn_compras.smn_lineas}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_cabecera_version_id");
			if (d != null)
			{
			name  = "b_version_headboard";
			label = "${lbl:b_version_headboard}";
		...
		d = rs.getString("rrs_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_contrato_id_name");
			if (d != null)
			{
			name  = "b_contract";
			label = "${lbl:b_contract}";
		...
		d = rs.getString("rrs_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
