package domain.repgen.smn_proceso.smn_impuest_deducc_detalle;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("smn_cod_impuesto_deducc_id");
			if (d != null)
			{
			name  = "b_code_tax_deduction";
			label = "${lbl:b_code_tax_deduction}";
		...
		d = rs.getString("smn_cod_impuesto_deducc_id_name");
			if (d != null)
			{
			name  = "b_code_tax_deduction";
			label = "${lbl:b_code_tax_deduction}";
		...
		d = rs.getString("smn_requisicion_detalle_id");
			if (d != null)
			{
			name  = "b_requisition_detail";
			label = "${lbl:b_requisition_detail}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("smn_requisicion_detalle_id_name");
			if (d != null)
			{
			name  = "b_requisition_detail";
			label = "${lbl:b_requisition_detail}";
