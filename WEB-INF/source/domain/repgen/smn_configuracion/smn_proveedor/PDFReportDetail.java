package domain.repgen.smn_configuracion.smn_proveedor;
...
public class PDFReportDetail extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		//SIMONE
				d = rs.getString("prv_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("prv_auxiliar_categoria_id_name");
			if (d != null)
			{
		...
		d = rs.getString("prv_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("prv_auxiliar_categoria_id");
			if (d != null)
			{
			name  = "b_auxiliary_category";
			label = "${lbl:b_auxiliary_category}";
