package domain.repgen.smn_configuracion.smn_contrato_modulo;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("ctm_fecha_registro_fin");
			if (d != null)
			{
			name  = "b_store_date";
		...
		d = rs.getString("smn_contrato_base_rf _name");
			if (d != null)
			{
			name  = "b_contract_base";
			label = "${lbl:b_contract_base}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
		...
		d = rs.getString("ctm_fecha_registro_ini");
			if (d != null)
			{
			name  = "b_store_date";
			label = "${lbl:b_store_date}";
		...
		d = rs.getString("ctm_dia_factura ");
			if (d != null)
			{
			name  = "b_day_fact";
			label = "${lbl:b_day_fact}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
