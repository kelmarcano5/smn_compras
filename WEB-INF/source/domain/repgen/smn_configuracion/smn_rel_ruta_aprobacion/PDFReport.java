package domain.repgen.smn_configuracion.smn_rel_ruta_aprobacion;
...
public class PDFReport extends AbstractPDFOutput
{
	...
	PdfPTable getReportFilter(GenericTransaction data) throws Throwable
	{
		...
		d = rs.getString("smn_lineas_id_name");
			if (d != null)
			{
			name  = "b_lines";
			label = "${lbl:b_lines}";
		...
		d = rs.getString("smn_ruta_id_name");
			if (d != null)
			{
			name  = "b_rute";
			label = "${lbl:b_rute}";
		...
		d = rs.getString("rra_aprobacion");
			if (d != null)
			{
			name  = "b_approbation";
			label = "${lbl:b_approbation}";
			str_filter = StringUtil.replace(label, label, _labels.getLabel(name, _language));
